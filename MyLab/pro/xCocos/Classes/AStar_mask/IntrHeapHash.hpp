...
template <class T, class T2, class TRAITS,HHDuplicatePolicy policy=hhdpReplaceOld>
class IntrHeapHash{
public:
  //default constructor
  ...
  const T* Find2(const T2& value) const
  {
	...
	while(ptr)
	{
		if(value.IsSameState(ptr->m_UserState))
		{
			return ptr;
		}
		ptr=(T*)ptr->ihsNextNode;
	}
	return 0;
  }

  T* Find2(const T2& value)
  {
	  uint32_t hashCode=value.getHashCode();
	  uint32_t hidx = hashCode%hashSize;
	  T* ptr=hash[hidx];
