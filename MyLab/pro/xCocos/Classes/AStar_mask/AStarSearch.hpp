...
#define AutoUseDynamicStepLimit			(50)		// 自动动态掩码步数限制
#define AutoUseDynamicDistanceLimit		(300)		// 自动动态掩码距离限制
...
protected:
	...
	struct Node : public IntrHeapHashNodeBase
	{
		...
		bool m_bUseDynamic;			// 使用动态掩码
      bool m_bUseDynamicEx;			// 使用扩展动态掩码
	...
	else
	{
      ...
      n->m_UserState->GetSuccessors( this, n->parent ? n->parent->m_UserState : NULL ,m_agentCellSize,m_staticObstacle, n->m_bUseDynamic,n->m_bUseDynamicEx);
				...
				for( int i=0;i<m_successNum; ++i )
				{
					...
					// int newg = 0;
					// 这里有很大提高空间，最大可提高5-6倍效率，怀疑是hash表效率问题，待深入
