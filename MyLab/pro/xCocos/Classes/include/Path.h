...
class Path
{
    public:
        ...
        void clear(void) { i_nodes.clear(); }
        PathNode const* GetNodes(uint32_t start = 0) const { return &i_nodes[start]; }
        float GetTotalLength() const { return GetTotalLength(0,Size()); }
        float GetTotalLength(uint32_t start, uint32_t end) const
        {
        ...
        float GetPassedLength(uint32_t curnode, float x, float y, float z)
        {
            float len = 0, xd, yd, zd;
        ...
        void Add(float x,float y,float z,uint32_t time)
        {
            PathNode node;
            node.x = x;
            node.y = y;
...
public:
    ...
    bool UpdateNode(uint32_t diff)
    {
        ...
        {
            uint32_t overTime = m_totalTime-m_nodeTime;
    ...
    bool GetFuturePos(uint32_t time,float& newX,float& newY,float& newZ) const
    {
        if (Empty()) return false;
        uint32_t curNode = m_curNode;
        ...
        uint32_t totalTime = m_totalTime+time;
        uint32_t nodeTime = m_nodeTime;
        unsigned int nodeCount = Size();
        for (curNode;curNode<nodeCount;++curNode)
        {
            uint32_t overTime = totalTime-nodeTime;
