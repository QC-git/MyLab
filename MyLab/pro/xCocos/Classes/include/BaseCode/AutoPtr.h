...
class CAutoPtr {
 public:
  ...
  ~CAutoPtr() { Release(); }
  CAutoPtr& operator=(T* ptr) {
    if (m_ptr && m_ptr != ptr) Release();
    m_ptr = ptr;
    return *this;
  }
  T* pop() {
 ...
 public:
  ...
  POINT_NO_RELEASE<T>* operator->() {
    ASSERT(m_ptr);
    return static_cast<POINT_NO_RELEASE<T>*>(m_ptr);
  }
  operator const T*() const { return m_ptr; }
  const POINT_NO_RELEASE<T>* operator->() const {
    ASSERT(m_ptr);
    return static_cast<POINT_NO_RELEASE<T>*>(m_ptr);
  }
...
inline void safe_release(CAutoPtr<T, T2>& pT) {
  pT.Release();
}
...
class CJavaObj {
 ...
 public:
  operator T*() {
    if (!m_ptr) m_ptr = T2::CreateNew(GUID_OF(T2));
    ASSERT(m_ptr);
    return m_ptr;
  }
  POINT_NO_RELEASE<T>* operator->() {
...
inline void safe_release(CJavaObj<T, T2>& pT) {
  pT.Release();
}
...
class CAutoRef {
 ...
 public:  // enable
  ...
  CAutoRef(const CAutoRef& ptr) { m_ptr = ptr.m_ptr; }
