...
public:
    ...
    explicit WorldPacket(uint16_t opcode, size_t res=2048) : ByteBuffer(res)//,m_extBuf(NULL)
    {
        append<uint16_t>((uint16_t)res);
        append<uint16_t>(opcode);
        //         IF_OK(res>4)
        //         {
        //             uint8_t v=0;
        //             append<uint8_t>(&v,res-4);
        //         }
    }
...
//         else
    ...
    void UpdateSize()
    {
        GetHead()->usSize = static_cast<USHORT>(size());
    }
...
protected:
    void Initialize(uint16_t opcode, size_t newres=1024)
    {
//         m_extBuf=NULL;
        clear();
        _storage.reserve(newres);
        append<uint16_t>((uint16_t)newres);
        append<uint16_t>(opcode);
        if(newres>4)
        {
            uint8_t v=0;
            append<uint8_t>(&v,newres-4);
        }
    }
// private:
//     char* m_extBuf;
